...
bool load_content() {
  ...
  // Need to rotate the teapot on x by negative pi/2
	meshes["teapot"].get_transform().orientation = rotate(quat(), -half_pi<float>(), vec3(1.0f, 0.0f, 0.0f));
  ...
  // White plane
	...
	meshes["plane"].set_material(testMat);
  // Red teapot
	testMat.set_diffuse(vec4(1.0f, 0.0f, 0.0f, 1.0f));
	meshes["teapot"].set_material(testMat);
  // *********************************
